    ...
    for (const QString& os :
        ...
        if (!candidates.isEmpty()) {
            for (QString& candidate : candidates) {
                candidate = RecipeCatalog::underItsMostSpecificFolder(candidate, os);
            }
...
QString RecipeCatalog::underItsMostSpecificFolder(const QString& tokenised, const QString& os) {
